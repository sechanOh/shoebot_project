...
def main():
    ...
    try:
        ...
        i = 0
        while True:
            distance = np.sqrt(np.float64(tvec[0])**2 + np.float64(tvec[1])**2 + np.float64(tvec[2])**2)
            distance = tvec[2]
            ...
            if distance <= threshold:
                ...
                agent.remember(state, pose)

                break
            
            i +=1
            if distance < threshold * 1.2:
                angle_gap = angle_gap_origin
            else:
                angle_gap = angle_gap_origin * 0.6
            ...
            best_pose = pose.copy()
            min_dist = distance
            prob_mean = best_pose.copy()
            bad_pose = []
            for _ in range(random_state_gen_num):
                goal_joint_angle[:4] = agent.generate_random_pose(pose, prob_mean, bad_pose)
                ...
                if (distance > min_dist):
                    bad_pose.append(present_joint_angle[:4])
                    for idx in range(1,4):
                        prob_mean[idx] = np.random.uniform(pose[idx] - angle_gap, pose[idx] + angle_gap)
                    continue
                for __ in range(3):
                    if (abs(tvec[0]) > 0.025 or abs(tvec[1]) > 0.045):
                        goal_joint_angle[3] = present_joint_angle[3] - angle_gap * 0.7
                        teleop_keyboard.send_goal_joint_space(path_time * 0.3)
                        wait_arrive()
                    else:
                        break
                print(tvec, ", ", distance)
                if (distance <= min_dist):
                    best_pose = present_joint_angle[:4].copy()
                    min_dist = distance

            # If fail to find best pose
            if best_pose == pose:
                goal_joint_angle[:4] = pose.copy()
                teleop_keyboard.send_goal_joint_space(path_time)
                wait_arrive()
                continue

            # remember best act
            agent.remember(state, best_pose)
            print(i, "th best pose:", best_pose)

            # move to next pose
            goal_joint_angle[:4] = best_pose.copy()
            teleop_keyboard.send_goal_joint_space(path_time)
            wait_arrive()
